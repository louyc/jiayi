...
import com.lifelight.api.entity.ApiRoleInfoExample;
...
import com.lifelight.dubbo.dao.ApiRoleInfoMapper;
...
public class ApiUserInfoServiceImpl implements ApiUserInfoService {
	...
	@Override
	public Result getApiUserInfoById(String managerId, String mobile, String openId, String headimgurl, String url,
			String city,Integer platformId) {
	...
	@SuppressWarnings("rawtypes")
	@Override
	public Result updateWxaUserInfo(ApiUserInfoVO apiUserInfo) {
		Result result = new Result<>(StatusCodes.OK, true);
		try {
			apiUserInfo.setUpdateTime(new Date());
			int i = apiUserInfoMapper.updateWxaUserinfoVO(apiUserInfo);
			if (i > 0) {
				apiUserInfo.setManagerId(apiUserInfo.getAccountManagerId());
				result.setResultCode(new ResultCode("SUCCESS", "完善资料成功！"));
				result.setData(apiUserInfo);
	...
	public Result getAllApiUserInfo(String managerId) {
		...
		if (StringUtil.isEmpty(user.getParentId())) { // 父账户
			...
			List<Map<String, Object>> listUser = apiUserInfoMapper.selectSubUserInfo(user.getParentId());
			for (Map<String, Object> userInfo : listUser) {
				ApiUserInfoVO apiUserInfoVO = new ApiUserInfoVO();
				listUserVO.add((ApiUserInfoVO) MapToObject.transMap2Bean(userInfo, apiUserInfoVO));
			}
	...
	public Result getParentUserInfo(String managerId) {
		...
		if (listUser.size() > 0) {
			ApiUserInfoVO apiUserInfoVO = new ApiUserInfoVO();
			apiUserInfoVO = (ApiUserInfoVO) MapToObject.transMap2Bean(listUser.get(0), apiUserInfoVO);
			result.setData(apiUserInfoVO);
			result.setResultCode(new ResultCode("SUCCESS", "查询成功！"));
		} else {
			result = new Result(StatusCodes.FAILED_DEPENDENCY, false, new ResultCode("QUERY_ZERO", "父账户未找到"));
		}
		return result;
	}
	...
	@SuppressWarnings("rawtypes")
	@Override
	public Result addSubAccount(ApiUserInfoVO userInfo, String managerId,Integer platformId) {
		Result result = new Result<>(StatusCodes.OK, true);
		try {
			...
			if (null != sonUser) {
				...
				sonUser.setPlatformId(platformId);
	...
	@Override
	public String getManagerIdByMobile(String mobile,Integer deviceId,String weixinId) {
		String managerId = null;
//		if(!StringUtil.isEmpty(mobile)) {
//			ApiUserInfo record = new ApiUserInfo();
//			record.setMobile(mobile);
//			record.setPlatformId(Integer.parseInt(weixinId));
//			managerId = apiUserInfoMapper.getManagerIdByMobile(record);
//		}
		if(StringUtil.isEmpty(managerId)) {
			DeviceUserRel deviceUserRel = new DeviceUserRel();
			deviceUserRel.setDeviceId(deviceId);
			DeviceUserRel ur = deviceUserRelMapper.checkDeviceIsBindingByManagerId(deviceUserRel);
			if(null!=ur) {
				managerId = ur.getManagerId();
			}
		}
		return managerId;
	}
