...
import com.lifelight.common.tools.util.UUIDUtil;
...
import com.lifelight.dubbo.dao.OrderInfoMapper;
...
public class OrderInfoServiceImpl implements OrderInfoService {
	...
	@Autowired
	public ApiUserInvoiceMapper apiUserInvoiceMapper;
	...
	public Result<OrderInfo> addOrderInfo(OrderInfo orderInfo) {
		...
		//是否开发票
		if(!StringUtil.isEmpty(orderInfo.getIsInvoice()) && orderInfo.getIsInvoice().equals("1")) { 
			ApiUserInvoiceVO invoice = apiUserInvoiceMapper.selectBytimeDesc(orderInfo.getUserId());
			if(null!=invoice) {
				orderInfo.setIsInvoice(orderInfo.getIsInvoice());
				orderInfo.setInvoiceType(invoice.getInvoiceType());
				orderInfo.setInvoiceTitleType(invoice.getInvoiceTitleType());
				orderInfo.setTaxpayer(invoice.getTaxpayer());
				orderInfo.setTaxpayerNumber(invoice.getTaxpayerNumber());
				orderInfo.setTicketCollectorEmail(invoice.getTicketCollectorEmail());
				orderInfo.setTicketCollectorMobile(invoice.getTicketCollectorMobile());
			}
		}
		//  计算订单总价
		...
		for(String userId : userIds){
			for(Integer serviceId : serviceIds){
				...
				*/
				...
				saveOrder.setPlatformId(platformId);
				// 保存数据
