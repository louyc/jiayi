...
import com.lifelight.api.vo.ProvinceVO;
...
import com.lifelight.dubbo.service.OperationDataService;
...
public class OperationDataServiceImpl implements OperationDataService {
	...
	public PaginatedResult<UserCountVO> signCount(ApiUserInfoVO userInfo) {
		...
		UserCountVO userCount = new UserCountVO();
		List<UserCountVO> listCout = new ArrayList<UserCountVO>();
		BackstageUserInfo user = userMapper.selectByPrimaryKey(userInfo.getManagerId());
		List<String> doctorIdList = new ArrayList<String>();
		if(user.getRoleId()==1) {//医生
			doctorIdList.add("'" + userInfo.getManagerId() + "'");
		}else if(user.getRoleId()==2) {//机构
			JyDoctorInfo entity = new JyDoctorInfo();
			entity.setSignOrgId(userInfo.getManagerId());
		...
		if (doctorIdList.size() > 0) {
			userInfo.setDoctorIdList(doctorIdList);
		}
	...
	public Result familyDetail(UserCountVO userInfo) {
		...
		for (String parentId : userInfo.getParentIds().split(",")) {
			...
			if (null != parentUser) {
				...
				List<ApiContractedUserVO> returnList = new ArrayList<ApiContractedUserVO>();
				...
				listUser = apiContractedUserMapper.selectAllByFamilyDetailListPage(userInfo);
