...
public class JyOrgDocRelServiceImpl implements JyOrgDocRelService {
	...
	@Override
	public Result<Message> inviteDoctor(JyOrgDocRel entity,Integer platformId) {
		...
		if(org != null && org.getType() != null ){
			...
			if("4".equals(org.getType()) ){
				// 邀请家庭医生
				result = JYInviteDoctor(entity,platformId);
			}else{
				// 邀请普通医生
				result = orgInviteDoctor(entity,platformId);
			}
		}else{	
	 ...
	 * @return
	 */
	public Result<Message> JYInviteDoctor(JyOrgDocRel entity,Integer platformId){
		...
		if(num > 0){
			logger.info("邀请成功 ！");
			//发送站内消息
			sendMessage(entity,result,platformId);
	 ...
	 * @return
	 */
	public Result<Message> orgInviteDoctor(JyOrgDocRel entity,Integer platformId){
		Result<Message> result = new Result<>(StatusCodes.OK, true);
		//判断有效期内医生是否签约
		JyOrgDocRel query = new JyOrgDocRel();
		query.setDoctorId(entity.getDoctorId());  // 医生id
	...
	public Result<Message> JYAgreeInvite(JyOrgDocRel entity,Integer platformId){
		Result<Message> result = new Result<>(StatusCodes.OK, true);
		//判断有效期内医生是否签约
		JyOrgDocRel query = new JyOrgDocRel();
		query.setValidTime(new Date());           // 有时间
		...
		try{
			for(JyOrgDocRel item : inviteSignList){
				if(item.getId() == entity.getId()){
					...
					//发送消息
					sendMessage(item,result,platformId);
				}else{
					// 拒绝其他机构
					item.setSignType(2);
					item.setUpdateTime(new Date());
					...
					jyOrgDocRelMapper.updateByPrimaryKeySelective(item);
					//发送站内消息
					sendInsideMessage(item,platformId);
				}
			}
			logger.info("签约成功 ！");
	...
	public Result<Message> doctorAgreeInvite(JyOrgDocRel entity,Integer platformId){
		Result<Message> result = new Result<>(StatusCodes.OK, true);
		// 签约医生，修改状态
		...
		//发送消息
		return sendMessage(entity,result,platformId);
	}
	
	@Override
	public Result<Message> agreeInvite(Integer relId,Integer platformId) {
		Result<Message> result = new Result<>(StatusCodes.OK, true);
		// 查询关系信息
		...
		if("4".equals(org.getType()) ){
			// 邀请家庭医生
			result = JYAgreeInvite(entity,platformId);
		}else{
			// 邀请普通医生
			result = doctorAgreeInvite(entity,platformId);
		}
	...
	@Override
	public Result<Message> refuseInvite(Integer relId,Integer platformId) {
		Result<Message> result = new Result<>(StatusCodes.OK, true);
		// 查询关系信息
		JyOrgDocRel entity = jyOrgDocRelMapper.selectByPrimaryKey(relId);
		if(entity == null || (entity != null && entity.getSignType() != 0) ){
	 ...
	 * @param entity
	 */
	private Message sendInsideMessage(JyOrgDocRel entity,Integer platformId){
	 ...
	 * @param entity
	 */
	private Result<Message> sendMessage(JyOrgDocRel entity , Result<Message> result,Integer platformId){
	
		// 发送站内消息
		Message message = sendInsideMessage(entity,platformId);
