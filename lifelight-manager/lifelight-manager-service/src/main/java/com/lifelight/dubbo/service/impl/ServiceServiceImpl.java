...
import com.lifelight.api.entity.UserShareService;
...
import com.lifelight.dubbo.dao.ServiceGoodsRelMapper;
import com.lifelight.dubbo.dao.ServiceInfoMapper;
import com.lifelight.dubbo.dao.UserShareServiceMapper;
...
public class ServiceServiceImpl implements ServiceService {
	...
	public Result getServiceInfoListPage(ServiceInfoVO serviceVO) {
		...
		try {
			...
			pa = new PaginatedResult<ServiceInfoVO>(listService, serviceVO.getCurrentPage(),
	...
	public PaginatedResult<ServiceInfoVO> getServiceInfoVoByKeywords(ServiceInfoVO serviceVO) {
		...
		if (0 < serviceVO.getCurrentPage() && 0 < serviceVO.getShowCount()) {
			...
			result = new PaginatedResult<>(serviceList, serviceVO.getCurrentPage(),
	...
	public Result queryGoodsById(String id) {
		...
		try {
			...
			for (Map<String, Object> serviceMap : listService) {
				// 服务关联的所有商品信息
				List<GoodsInfo> goodsInfoList = serviceGoodsRelMapper
						.selectGoodsById(Integer.parseInt(id));
				...
				// 服务关联的所有标签信息
				List<Keywords> keywordsList = keywordsRelMapper
						.selectListByRelationId(Integer.parseInt(id), 1);
				if (null != keywordsList && keywordsList.size() > 0) {
					for (Keywords keywords : keywordsList) {
						keywordsName = keywordsName + "," + keywords.getName();
						keywordsIdList = keywordsIdList + "," + keywords.getId();
					...
					serviceMap.put("keywordsIdList",
							keywordsIdList.substring(1, keywordsIdList.length()));
				}
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public Result queryAllServiceInfo(String createUserId,Integer platformId) {
		Result result = new Result<>(StatusCodes.OK, true);
		...
		serviceInfo.setPlatformId(platformId);
