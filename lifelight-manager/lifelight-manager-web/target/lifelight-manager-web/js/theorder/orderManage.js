    ...
    layui.use(['laypage', 'layer','form','laydate'], function () {
        ...
        var start = {
	    	...
	    	function getOrderList(pageNum){
	        	...
	        	dataObj.mailStatus = $('#mailStatus').val()=='all' ? '' : $('#mailStatus').val();
	        		...
	        		if(res.successful){
	        			...
	        			if(data.length>0){
	        				$.each(data,function(index,item){
	        					...
	        					str+='<td><button class="layui-btn layui-btn-small viewBtn" orderType="'+item.type+'">查看</button>';
	        					if(item.payStatus == '1'){
	        						if(item.mailStatus == '1'){
		        						str+='<button class="layui-btn layui-btn-small sendGoods">发货</button>';
		        					}else{
		        						str+='<button class="layui-btn layui-btn-small layui-btn-disabled">已发货</button>';
		        					}
	        					}
	        					str+='</td></tr>';
//					        str+='<td><button class="layui-btn viewBtn '+(item.type==1?'layui-btn-disabled" disabled>':'">')+'查看</button></td></tr>';
        ...
        //获取订单详情
        $('.list-table').delegate('.sendGoods','click',function(){
        	...
        	$.ajax({
	        	...
	        	success:function(res){
	        		if(res.status=="200" && res.successful){
	        			layer.msg("发货成功！",{time: 2000,zIndex: layer.zIndex},function(){
        ...
        $('.list-table').delegate('.viewBtn','click',function(){
    		...
    		$.get('/orderInfo/getOrderDetailById?id='+id,function(res){
    			...
    			if(data.isInvoice == "1"){
    				...
    				$('#invoice-info-box .taxpayer').html(data.taxpayer);
    				$('#invoice-info-box .taxpayerNumber').html(data.taxpayerNumber);
    			}else{
