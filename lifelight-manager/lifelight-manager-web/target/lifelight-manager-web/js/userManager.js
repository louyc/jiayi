window.onload = function () {
	...
	//	            				str += '</td></tr>';
		            			...
		            			if(item.inUse==0){
		            				str += '<button class="layui-btn layui-btn-smal layui-btn-disabled checkQRCode">二维码</button>';
		            			}else{
		            				/*if(){
		            					
		            				}*/
		            				str += '<button class="layui-btn layui-btn-smal checkQRCode">二维码</button>';
		            			}
		            			str +='</td></tr>';
		            		})
	            			laypage({
...
//	                        		pagging();
	...
	//	        	        userStatus = $('#audit-status').val();
        ...
        //查看二维码
        $('#userList').delegate('.checkQRCode','click',function(){
        	var obj =new Object();
    		obj.managerId=$(this).parents('tr').attr('managerId');//这里是医生Id
    		$.ajax({
    			url:'/qrcode/createDoctorQrcodeInfo',
    	        type:'POST',
    	        data:$.toJSON(obj),
                dataType:'json',
                contentType: 'application/json',
    	        success : function(data){
    	        	if(data.successful){
    	        		var item = data.data;
    	        		var str = "";
    	        		str += '<img alt="生成二维码失败" style="height: 300px;width: 300px;margin-top: 20px;" src="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket='+item.ticket+'">'
    	        		str += '<div class="codeHandle"><a href="/qrcode/download?ticket='+item.ticket+'&name='+item.name+'" ticket="'+item.ticket+'">下载</a></div>';
    	        		$("#QRCode").html(str);
    	        		layer.open({
    	            		type:1
    	            		,title:'查看二维码'
