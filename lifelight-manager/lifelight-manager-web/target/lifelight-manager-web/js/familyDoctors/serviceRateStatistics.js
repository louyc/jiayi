$(function(){
    layui.use(['laypage', 'layer','form','laydate'], function () {
    	...
    	function loadTable(){
    		...
    		$.ajax({
    			url: "/statistics/serviceStatistics?startDate="+startDate+"&endDate="+endDate+"&orgId="+managerId,
    			type: "get",
    			success : function(res){
    				...
    				if(res.data.length <= 0){
    					...
    					var tr1 = '<th rowspan="2">分组</th>'+
			                ...
			                '<th rowspan="2">已完成数(包)</th>'+
			                '<th rowspan="2">服务包完成率(%)</th>';
    					var tr2 = "";
    					var tdStr = "<td>"+ res.data[0].orgName+"</td><td>小计</td><td>"+res.data[0].countSum+"</td><td>"+res.data[0].finishSum+"</td><td>"+res.data[0].finishRate+"</td>";
    					for(var item of res.data[0].list){
    						tr1 += "<th colspan='3' style='text-align: center'>"+item.itemName+"</th>";
    						tr2 += "<th>有偿签约数</th>";
    						tr2 += "<th>已完成数(个)</th>";
    						tr2 += "<th>执行率(%)</th>";
    						tdStr += "<td>"+item.countSum+"</td>";
    						tdStr += "<td>"+item.finishSum+"</td>";
    	...
    	$("#exportBtn").on("click",function(){
    		...
    		$(this).attr('href','/statistics/exportServiceStatistics'+query);
    	})
    })
})
