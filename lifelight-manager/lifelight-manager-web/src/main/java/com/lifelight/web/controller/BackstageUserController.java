...
import com.lifelight.api.vo.UserInfoVO;
...
import com.lifelight.web.Interceptor.AutorizadorInterceptor;
import com.lifelight.web.controller.limit.RequestLimit;
...
public class BackstageUserController {
	...
	public Result login(HttpServletRequest request, HttpServletResponse response,
		...
		Result result = managerService.login(userName, password, Integer.parseInt(platformId));
		if (result.isSuccessful()) { // 已在账户中心注册
			...
			if (!backstageResult.isSuccessful() && backstageResult.getData() == null) {
				...
				backstageUserInfo.setUserStatus(0);
				// 初始化信息
				backstageUserInfoService.backstageUserRegist(backstageUserInfo);
				backstageResult.setResultCode(new ResultCode("ACCOUNT_EXISTS", "已在账户中心注册,完善信息直接登录！"));
	...
	public Result regist(HttpServletRequest request, @RequestBody UserInfoVO info) {
		...
		if (StringUtils.isEmpty(info.getMobile()) || StringUtils.isEmpty(info.getRandCode())) {
			return new Result(StatusCodes.OK, false, new ResultCode("PARAM_ERROR", "参数错误！手机号或验证码不能为空"));
		}
	...
	@ResponseBody
	public Result updatePassword(HttpServletRequest request, @RequestParam("oldPassword") String oldPassword,
	...
	public Result sendPasswordRandomCode(HttpServletRequest request, @RequestParam String mobile) {
		...
		if (StringUtils.isEmpty(randCode)) {
			...
			redisTool.addMap("accountPassword", mobile, randCode, DateTimeUtil.modifyMinutes(new Date(), 5));
			// send
	...
	public Result forgetPassword(HttpServletRequest request, @RequestParam("mobile") String mobile,
		...
		return managerService.forgetPassword(mobile, password, Integer.parseInt(weixinId));
	}
	...
	@ResponseBody
	public Result resetPassword(HttpServletRequest request, @RequestParam("userName") String userName) {
		// 查询当前域名微信配置表id
		logger.info("域名：：：：" + request.getServerName());
		String domainName = request.getServerName();
		String weixinId = redisTool.get(domainName);
	...
	public Result update(HttpServletRequest request, @RequestBody BackstageUserInfo info) {
		...
		info.setDepts(null);
		if (!StringUtil.isEmpty(info.getTags()) && info.getTags().length() > 0 && info.getTags().indexOf("$") != -1) {
			tagArr = info.getTags().split("\\$");
		} else if (!StringUtil.isEmpty(info.getTags()) && info.getTags().indexOf("$") == -1) {
			tagArr = new String[] { info.getTags() };
		}
		info.setTags(null);
		tagsRelService.removeRel(info.getManagerId(), Integer.parseInt(weixinId));
		departmentRelService.removeRel(info.getManagerId());
		if (deptArr.length != 0) {
			departmentRelService.insertDepts(deptArr, info.getManagerId());
		}
		if (tagArr.length != 0) {
			tagsRelService.insertTags(tagArr, info.getManagerId(), Integer.parseInt(weixinId));
		}
		// 设置初始审核状态
	...
	@ResponseBody
	public Result deployUserRole(HttpServletRequest request, @RequestParam("userId") String userId,
			@RequestParam("roleId") int roleId) {
		// log
		logger.info("BackstageUserController.deployUserRole start,userId={},roleId={}", userId, roleId);
